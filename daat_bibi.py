"""
Processamentos dos dados das BiBis com a 
tecnica DAATs
"""
...
dire  = np.zeros((10,1)) #direcao (2 valores, ate 5 faixas)
espe  = np.zeros((10,1)) #espectros (2 valores, ate 5 faixas)
energ = np.zeros((10,1)) #Hs + 5 energias (uma por faixa), 4 picos (maiores)
#dire1 = np.copy(dire)
#espe1 = np.copy(espe)
...
# mm=[64,50,48,44,38,32,30,27,25,24,21,19,18,17,16,15,14,13,12,11,11,10,10,9,9,8,8,8,7,7,7,7,6,6,6]

mm = [int(np.round(2.1*3)), int(np.round(1.6*3))]

ms=[]

#cria vetores de dim 64,34
wavecos = np.zeros((max(mm),len(mm)))
...
# for ik in range(len(listap)):
    ...
    # if len(dd) >= 1024 and dd[:,1].max() > 0.5:
        ...
        # ano = int(listap[ik][0:4])
        # mes = int(listap[ik][4:6])
