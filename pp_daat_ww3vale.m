...
%ondas
% pathname = '/home/lioc/Dropbox/lioc/dados/pnboia/triaxys/rio_grande/HNE/';
pathname = '/home/lioc/Dropbox/ww3vale/Geral/TU/rot/out/bruto/series/';
% pathname = 'C:\Users\hp\Dropbox\ww3vale\Geral\TU\rot\out\bruto\series/';

% pathnamelist = '/home/lioc/Dropbox/lioc/dados/pnboia/triaxys/listas/';

%carrega lista com nome dos arquivos em string
arqp = textread([pathname,'lista_adcp3_abr13.txt'],'%s');
adcp = '3';
%carrega listas da axys
% arq = load([pathname,'lista.txt'],'%s');
% arqp = num2str(arq);

mes = 04;
ano = 2013;
local = 'ADCP 3 - Porto de Tubarao/ES';

%declinacao magnetica
%dmag = -22; %santos
% dmag = -17; %rio grande
dmag = -23.5; %vitoria %%%+90 corrige a convencao

%carrega vento do anemometro
% vento = dlmread('/home/lioc/Dropbox/ww3vale/Geral/TU/rot/out/proc/anemo_vale.out',',',1,0);
% 
% ini = find(vento(:,1)==201302010000);
% fim = find(vento(:,1)==201302282350);
% 
% ws = vento(ini:fim,2);
% wd = vento(ini:fim,3);

%vento - cfsr
% u = load('/home/lioc/Dropbox/cfsr/uCFSR_RioGrande_200912.txt');
% v = load('/home/lioc/Dropbox/cfsr/vCFSR_RioGrande_200912.txt');
% 
% ws = sqrt(u.^2 + v.^2);
...
%%% verificar o comprimento do vetor (fazer dentro do loop)
% fs = 1; %0.78;
% nfft = 64; %64;
...
%vetor de frequencias (verificar se esta correto - para o adcp esta dentro do loop
%devido a teram arquivos com 1024 e 2048 pontos)
...
for ik = 1:ik
    ...
    arq1=[pathname,char(arqp(ik))]; 
    
    %eta, etaNS, etaEW (heave, vx, vy)
    mat=load(arq1);
    ...
    disp(length(co))
     ...
     %como nos dados dos adcps tem aquivos com 1024 e outros com 2048
     %pontos, essa condicao escolhe os parametros para o calculo do
     %espectro com base no comprimento do vetor
    if length(co) == 1024
        fs = 1;
        nfft = 64;
        ...
        faixa1 = [3:5]'; % 21.3 - 16 s
        faixa2 = [5:7]'; % 12.8 - 9.1 s
        faixa3 = [8:10]'; % 8 - 6.4 s
        faixa4 = [11:16]'; % 5.8 - 4 s    

    elseif length(co) > 1024
        fs = 2;
        nfft = 128;
        %vetor de frequencias (verificar se esta correto)
        dt = 1 / fs; %intervalo de amostragem
...
%     %cria faixas de freq (periodo) %%% foi colocado dentro da condicao
%     que varia com o comprimento do vetor
%     faixa1 = [3:7]'; % 32 - 10.6 s
%     faixa2 = [7:14]'; % 9.1 - 7.1 s
%     faixa3 = [14:21]'; % 6.4 - 4.2 s
%     faixa4 = [21:31]'; % 4 - 3.2 s    
...
if adcp == '4'
    aux = find(energ1(1,:)>1); %pega valores com hs > 1
    espe1(:,aux) = NaN;
end

if adcp == '2'
    dire1 = dire1 - dmag;  % + 45;  %correcao de direcao
    espe1(find(dire1 < 90)) = NaN;  %limpa ruido
    espe1(find(dire1 > 200)) = NaN;
end
