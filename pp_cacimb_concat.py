...
pathname_mod3 = os.environ['HOME'] + '/Dropbox/ww3es/Geral/modelagem/resultados/Cacimbas/Cacimbas_3Grades/'
...
#figuras
pl.subplot(3,1,1)
pl.plot(datam,matondam[:,1],'-b',datat,matondap[:,1],'-r')
pl.ylabel('Hs (m)',fontsize=12), pl.grid()
pl.legend(['WW3','ADCP'])
pl.ylim(0,4)

pl.subplot(3,1,2)
pl.plot(datam,matondam[:,2],'.b',datat,matondap[:,2],'.r')
pl.ylabel('Tp (s)',fontsize=12), pl.grid()
pl.legend(['WW3','ADCP'])
pl.ylim(0,20)

pl.subplot(3,1,3)
pl.plot(datam,matondam[:,3],'.b',datat,matondap[:,3],'.r')
