...
iday=1
ihour=0
# Input Time interval (hours)
dt=1 
# Output Time interval (hours)
odt=24 # One per day
# ---------------------------------------------------------------------------------------
# Flag for creating ice figures. cfig=1 create. cfig=0 does not create and save run time
cfig=1;
# ---------------------------------------------------------------------------------------------------------------
# For running program using just one netcdf input file, write the name here. Otherwise comment the following line
name='icethk.gdas.199407.nc';tn=1
rname='CFSR'
...
fl.write('Took the latitudes and longitudes related to ice grid arguments.\n')
fl.write('------------------------------------------------------------------------------\n')
...
for tn in range(0,tn):
	...
	#for t in range(0,2):
		...
		vf.write('\n')
		# write ice coverage using WW3 format
		np.savetxt(vf,flipud(nicec[:,:]),fmt="%5.1f",delimiter='')   
		...
		if cfig==1:
			...
			map.contourf(xx,yy,nicec,levels,cmap=palette,norm=colors.normalize(vmin=-1,vmax=1.5,clip=False))
