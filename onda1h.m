...
prompt={'Os dados sao direcionais (entre com o valor 1) ou nao-direcionais (entre com o valor 0):',...
        'BMO-BR (digite 6), Wavescan-GOM (digite 5),Axys (digite 4), Boia do Merenda no ES (digite 3), Waverider (digite 2),da P-18 (digite 1) ou a do SEGEN/CONSUB (digite 0), deixe em branco caso seja nao-direcional:',...
        ...
        'Entre com o ID (6 digitos - default = 999999):',...
        ...
        'Valor da Altura Significativa minima para tentar identificar mais de um pico',...
        'Tratamento direcional 1D (digite 1). Tratamento direcional 1D e 2D com Metodo de Maxima Entropia (digite 2)'};
%def={'1',' ',' ','1','999999',' ',' ',' ',' ','32','-23','1e3','0.30','2'}; %Padroes para qualquer dado de boia.
%def={'1','3',' ','1','999999','-19.95','-39.53','2006',' ','32','-23','1e3','0.30','2'}; %Padroes para boia do ES (Merenda).
%def={'1','4',' ','0.78','999999','-24.39','-44.05','2009',' ','32','-23','1e3','0.30','2'}; %Padroes para a boia Axys 13 BS.
def={'1','4',' ','0.78','999999','-23.128','-40.920','2011',' ','32','-23','1e3','0.30','2'}; %Padroes para a boia Axys .
%def={'1','4',' ','0.78','999999','-23.13','-40.92','2008',' ','32','-23','1e3','0.30','2'}; %Padroes para a boia Axys .
%def={'1','5',' ','0.5','091007','26.66792','-90.46892','2011',' ','32','0.5','1e3','0.30','2'}; %Padroes para a boia Wavescan GOM.
%def={'1','2',' ','0.78','999999','-27.7','-48.1','2001',' ','32','-23','1e3','0.30','2'}; % Waverider
%def={'1','6',' ','1','999999','-23.057','-44.2343','2011',' ','32','-23','1e3','0.10','2'}; %Padroes para a boia BMO_BR
...
%========================================================================
%header abaixo sem ultimas colunas com parametros de spreading (PHI,N_PHI,S_PHI,N_KROG,S_KROG)
...
% 
%header abaixo com ultimas colunas com parametros de spreading (PHI,N_PHI,S_PHI,N_VSPR,S_VSPR)
...
%header2={'%      DATA';' HORA';'      ID';'        LAT';'        LON';'    SDVH';'   VAVG';'   VAVH';'   VTZD';'   VTZM';'   VTZS';'   VZMX';'    VDMX';'          VMTA';'          VMTB';'          VMTC';'          VMTD';'          VMTE';'     VTPK';'      VSPK';'    VMED';'       VSPR';'    VPED';'         VMTA1';'         VMTB1';'         VMTC1';'         VMTD1';'         VMTE1';'    VCAR1';'    VTPK1';'     VSPK1';'      VSPR1';'   VPED1';'      ALFA1';'  GAMA1';'         VMTA2';'         VMTB2';'         VMTC2';'         VMTD2';'         VMTE2';'    VCAR2';'    VTPK2';'     VSPK2';'      VSPR2';'   VPED2';'      ALFA2';'  GAMA2';'         VMTA3';'         VMTB3';'         VMTC3';'         VMTD3';'         VMTE3';'    VCAR3';'    VTPK3';'     VSPK3';'     VSPR3';'   VPED3';'      ALFA3';'  GAMA3';'  PHI';'  S_PHI';'  S1VSPR';'  S2VSPR';' S1VSPR1';' S2VSPR1';' S2VTPK1';' S1VSPR2';' S2VSPR2';' S2VTPK2';' S1VSPR3';' S2VSPR3';' S2VTPK3'};
...
while ~isempty(arqdwv) % Teste para identificar se ainda existe algum arquivo
  ...
  if pdir == 1
    ...
    if dboia == 4 
      ...
      [heave,etaEW,etaNS,ano,mes,dia,hora,min,sdata,stime]=le_axys(arqdwv); % mudar o nome para le_boiaAxys(arqdwv)
    end
    ...
    if dboia == 6
        % Leitura dos dados da boia BMO-BR (Andre Mendes)
  ...
  %========================================================================
   if pdir
    ...
    % Ja foram referenciadas para o referencial da trat_dir_EMEM.
    ...
    %[VD,VPED,VNUM,VESP,VSPR,A1,B1,A2,B2,Rcheck] = trat_dir_fft_tucker1989(VF,noverlap,w,heave,etaEW,etaNS,DT,DONV,kVTPK,nfft,dboia,prof); % Nao apresentou bons resultados!!! Funcao igual ao artigo do Tucker 1989.
