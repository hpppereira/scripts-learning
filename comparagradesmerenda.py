# -*- coding: utf-8 -*-
'''
Programa principal para avaliacao entre
os resultados modelos usando 2 ou 3 grades
'''
...
pathname_mod3 = os.environ['HOME'] + '/Dropbox/ww3es/Geral/modelagem/resultados/Merenda/'
direm = np.sort(os.listdir(pathname_mod3))

#caminho dos resultdaos da modelagem
pathname_mod2 = os.environ['HOME'] + '/Dropbox/ww3es/Geral/modelagem/resultados/Merenda_2g/'
direm2 = np.sort(os.listdir(pathname_mod2))
...
for dto in direm:
    ...
    dadosm2 = np.loadtxt(pathname_mod2 + dto + '/tab_southatl/tab50.ww3',skiprows=4,usecols=(0,1,4,9,10))
...
matondam2= np.array([datami,ddm2[:,2],1./ddm2[:,3],ddm2[:,4]]).T
...
matondam2 = matondam2[indi_mod:indf_mod,:]
...
#figuras
pl.subplot(3,1,1)
pl.plot(datam,matondam[:,1],'-b',datam,matondam2[:,1],'-k',datat,matonda[:,1],'-r')
pl.ylabel('Hs (m)',fontsize=12), pl.grid()
pl.legend(['3Grades','2Grades','BRMerenda'])
pl.ylim(0,5)

pl.subplot(3,1,2)
pl.plot(datam,matondam[:,2],'.b',datam,matondam2[:,2],'.k',datat,matonda[:,2],'.r')
pl.ylabel('Tp (s)',fontsize=12), pl.grid()
pl.legend(['3Grades','2Grades','BRMerenda'])
pl.ylim(0,20)

pl.subplot(3,1,3)
pl.plot(datam,matondam[:,3],'.b',datam,matondam2[:,3],'.k',datat,matonda[:,3],'.r')
