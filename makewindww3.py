...
name='wnd10m.gdas.200912.nc';tn=1
rname='CFSR'
...
fl.write('Latitudes:  Initial: '+repr(round(lat[alati],6))[0:10]+'   Final: '+repr(round(lat[alatf],6))[0:10]+'    Length='+repr(lat[alati:alatf].shape[0])+'   \n')
fl.write('Longitudes:  Initial: '+repr(round(lon[aloni],6))[0:10]+'   Final: '+repr(round(lon[alonf],6))[0:10]+'    Length='+repr(lon[aloni:alonf].shape[0])+'  \n')
fl.write('------------------------------------------------------------------------------\n')
...
for tn in range(0,tn):
	...
	for t in range(0,time.shape[0],dt):
	#for t in range(0,2):
		...
		uwnd=fu.variables['UGRD_10maboveground'][t,alati:alatf,:]
		vwnd=fu.variables['VGRD_10maboveground'][t,alati:alatf,:]
		...
		np.savetxt(vf,flipud(uwnd[:,aloni:alonf]),fmt="%10.3f",delimiter='')   
		# write V component of wind using WW3 format
		np.savetxt(vf,flipud(vwnd[:,aloni:alonf]),fmt="%10.3f",delimiter='')   
		...
		#	map.warpimage('/usr/share/basemap/data/shadedrelief.jpg') 
...
#			map.drawparallels(np.arange(round(lat[0]),round(lat[-1]),int((lat[-1]-lat[0])/5)),labels=[1,0,0,0])
                        map.drawparallels(np.arange(-80,81,10),labels=[1,0,0,0])
                        map.drawmeridians(np.arange(round(lon[0]),round(lon[-1]),10),labels=[0,0,0,1])

		#	latc=[-23.94,-22.9,-21.75]
		#	lonc=[-46.34 ,-43.233,-41.2]
		#	cities=['Santos, SP','Rio de Janeiro, RJ','Campos, RJ']
			# Wind Intensity 
			magn=sqrt((uwnd[:,aloni:alonf]*uwnd[:,aloni:alonf])+(vwnd[:,aloni:alonf]*vwnd[:,aloni:alonf]))    

			map.imshow(magn,palette,norm=colors.normalize(vmin=vmin,vmax=vmax,clip=False))
			ax = plt.gca()
			pos = ax.get_position()
			l, b, w, h = pos.bounds
			cax = plt.axes([l+w, b, 0.03, h]) # setup colorbar axes.
			plt.colorbar(cax=cax) # draw colorbar
			plt.axes(ax)  # make the original axes current again
			latb=[-25.283,-28.500,-31.566]
	   		lonb=[-44.933,-47.366,-49.866]
   			xb,yb = map(lonb,latb)
			map.plot(xb,yb,'bo',markersize=10)
			cities=['Boia'] 
			#for name,xpt,ypt in zip(cities,xb,yb): 
			 #   plt.text(xpt+1,ypt-1,name,fontsize=16,fontweight='bold')              
			# Plot vectors over shaded intensity fields 
			Q = map.quiver(lon[aloni:alonf:svec],lat[::svec],uwnd[::svec,aloni:alonf:svec],vwnd[::svec,aloni:alonf:svec],scale=300)
      
			title('Campo de vento a 10m (m/s) '+rname+'  '+repr(iyear)+rmonth+rday+'  '+rhour+' Z')
