...
pathname = os.environ['HOME'] + '/Dropbox/pnboia/dados/CHM/Validados/'
...
rg['date'] = [dt.datetime(rg.Year[i],rg.Month[i],rg.Day[i],rg.Hour[i]) for i in range(len(rg))]
fl['date'] = [dt.datetime(fl.Year[i],fl.Month[i],fl.Day[i],fl.Hour[i]) for i in range(len(fl))]
sa['date'] = [dt.datetime(sa.Year[i],sa.Month[i],sa.Day[i],sa.Hour[i]) for i in range(len(sa))]
re['date'] = [dt.datetime(re.Year[i],re.Month[i],re.Day[i],re.Hour[i]) for i in range(len(re))]
ps['date'] = [dt.datetime(ps.Year[i],ps.Month[i],ps.Day[i],ps.Hour[i]) for i in range(len(ps))]
cf['date'] = [dt.datetime(cf.Year[i],cf.Month[i],cf.Day[i],cf.Hour[i]) for i in range(len(cf))]
...
# rg.loc['2012-02-01 01:21:00':'2012-06-30 23:21:00',['wvht','dpd','mwd','wspd','wdir']].to_csv('out/RIG_Argos.csv', header=['hs','tp','dp','ws','wd'])
# fl.loc['2012-02-01 01:21:00':'2012-06-30 23:21:00',['wvht','dpd','mwd','wspd','wdir']].to_csv('out/FLN_Argos.csv', header=['hs','tp','dp','ws','wd'])
# sa.loc['2012-02-01 01:21:00':'2012-06-30 23:21:00',['wvht','dpd','mwd','wspd','wdir']].to_csv('out/SAN_Argos.csv', header=['hs','tp','dp','ws','wd'])
...
# rg[['wvht','dpd','mwd','wspd','wdir']].to_csv('out/RIG_Argos.csv', header=['hs','tp','dp','ws','wd'])
# fl[['wvht','dpd','mwd','wspd','wdir']].to_csv('out/FLN_Argos.csv', header=['hs','tp','dp','ws','wd'])
# sa[['wvht','dpd','mwd','wspd','wdir']].to_csv('out/SAN_Argos.csv', header=['hs','tp','dp','ws','wd'])
