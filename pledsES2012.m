%programa para plotagem da evolu��o do espectro direcional
%Bacia ES
...
arq2=[[1 0 0];[1 0 0];[1 0.55 0];[1 0.55 0];[1 1 0];...
      [1  1 0];[0 1 0];[0 1 0];[0.7 0.7 1];[0.7 0.7 1]];
col=[0.7 0.7 0.7];
text(2.5,14,'direcao verdadeira em graus - onda e vento','fontsize',12,'color','b');

set(gcf,'paperposition',[-0.5 -0.5 9.5 10.5]);
%codigo de cores para faixas de periodos
x=[0.5;2;2;0.5]+0.5;y=[2;2;10;10];
patch(x,y,arq2(1,:));
x=x+1.30;patch(x,y,arq2(3,:));
x=x+1.30;patch(x,y,arq2(5,:));
x=x+1.30;patch(x,y,arq2(7,:));
x=x+1.30;patch(x,y,arq2(9,:));
x=x+1.30;patch(x,y,[1 1 1]);
x=x+1.30;patch(x,y,[1 1 1]);
%y=y+16;patch(x,y,[1 1 1]);
%y=y+16;patch(x,y,[1 1 1]);
%y=y+16;patch(x,y,[1 1 1]);

text(1,290,['ESPECTRO DIRECIONAL DE ONDAS - ES - ' ad(mes,:) '/' num2str(ano)],...
    'fontsize',14,'color','b')
%ad(mes,:) '/' num2str(ano)]); 
...
a=2.9+10;
for i=1:9,a=a+8;
  text(.73,a,num2str(i),'fontsize',12,'fontweight','b');
  text(19.1,a,num2str(i),'fontsize',12,'fontweight','b');
end;

%dias do mês, eixo vertical de 10 a 31;
for i=10:31,a=a+8;
  text(.57,a,num2str(i),'fontsize',12);
  text(19.1,a,num2str(i),'fontsize',12);
end;
...
for i=1:18,
  text(i+0.15,18,num2str(a(i)),'fontsize',14,'color','r','fontweight','bold');
end;
ld=['NW';'N ';'NE';'E ';'SE';'S ';'SW';'W ';'SW';'W '];
k=1.55;
d=1.75:2.25:13;
 for i=1:8,
   text(k,21.1,ld(i,:),'fontsize',14,'color','k','fontweight','bold');
   k=k+2.25;
 end;
text(1.15,40,'dia do mes','fontsize',12,'rotation',90);ddir(5,:)=ddir(9,:);
...
set(gcf,'paperOrientation','portrait','paperposition',[1 1 15 15],...
     'paperSize',[15 15]);
print -dpcx256 pledsboiaES_welch