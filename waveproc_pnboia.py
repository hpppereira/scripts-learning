...
class WaveProc(object):
    ...
    def listdir(self):
        ...
        ''' Lista arquivos com extensao que estao dentro
         do diretorio 'pathname' 
        ...
        self.filelist = np.sort(os.listdir(self.pathname))
        

    def read_HNE(self, filename, fs, nfft, h):

        ''' Retorna os dados de tempo, elevacao e
           deslocamentos norte e leste

        Entrada : filename = Ex: 200907060200.HNE

        Output  : self.t     - time (sec)
        		  self.eta   - heave (vertical) (m)
        		  self.etax  - displacement EW  (m)
        		  self.etay  - displacement NS  (m)
        		  self.fs    - sample frequency (Hz)
        		  self.nfft  - window for fft   
        		  self.f     - vector frequency (Hz)
                 '''
        ...
        self.t, self.n1, self.n3, self.n2 = np.loadtxt(self.pathname + self.filename,
         skiprows = 11, unpack=True)

        self.date = pd.to_datetime(self.filename, format='%Y%m%d%H%M.HNE')
    
        #f = np.fft.fftshift(np.fft.fftfreq(len(self.n1) / (len(self.n1) / self.nfft), 1./self.fs))
        #self.f = f[len(f)/2:]

        self.dt = self.t[2] - self.t[1]
        #self.df = self.f[2] - self.f[1]
    ...
    def espec1(self, x):
        ...
        self.aa = np.array([self.f,sp,ici,ics]).T
