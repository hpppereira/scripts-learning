...
for ik in range(0,len(av),3):
    ...
    if ik == 253:
        ...
        #correcao - aceleracao para heave
#        qq1 = qq1 / ( (2 * pi * f1) ** 4 )
        ...
        faixa1 = np.array([2,3,4])
        faixa2 = np.array([4,5,6,7,8])  
        faixa3 = np.array(range(8,14))
        faixa4 = np.array(range(16,20))
...
# espe1 = espe[:,:espe.shape[1]/3]
# dire1 = dire[:,:dire.shape[1]/3]
# energ1 = energ[:,:energ.shape[1]/3]

#remove a faixa 1 (deixa valores zerados)
#espe[[0,1,4,5,6,7],:] = 0
...
dire1 = dire[:,::3] + dmag
energ1 = energ[:,::3]
ws1 = wind.ix[:-1:3,1].values
wd1 = wind.ix[:-1:3,0].values + dmag
...
#chama a pleds
pleds.pleds(espe1 * 30 ,dire1, ws1, wd1)
