...
mat = scipy.io.loadmat(pathname + 'mar1_gx3_py.mat')
...
#para processar todos os arquivos
p0 = 0
p1 = len(lista)

h = 1000 #profundidade 
nfft = 12191 #numero de dados para a fft (para nlin=1312 -- p/ 32gl, nfft=82 ; p/8 gl, nfft=328)
fs = 100 #freq de amostragem
nlin = 195056 #comprimento da serie temporal a ser processada
gl = (nlin/nfft) * 2
...
for i in range(len(listap)):
    ...
    #dados, data = carrega_axys.dados_hne(pathname,listap[i]+'.HNE')

    eta = mat.values()[:][4][:,0]
    etay = mat.values()[:][2][:,0]
    etax = mat.values()[:][5][:,0]
    t = np.arange(0,len(eta),1./100)
    ...
    #declara variaveis de deslocamento
    # dspx = etax
    # dspy = etay

    #tira zeros da serie de heave
    # eta[np.where(eta==0)] = 0.001

    #tentativa de calcular as inclinacoes
    # etax = np.arctan(etax/eta) * 180 / np.pi
    # etay = np.arctan(etay/eta) * 180 / np.pi

    # etax = np.arctan2(etax,eta) * 180 / np.pi
    # etay = np.arctan2(etay,eta) * 180 / np.pi

#corrigir angulos de inclinacoes
    # for i in range(len(etax)):

    #     if etax[i] > 0 and etax[i] < 90:
    #         etax[i] = etax[i] * -1

    #     if etay[i] > 0 and etay[i] < 90:
    #         etay[i] = etay[i] * -1

    #     if etax[i] < -90 and etax[i] > -180:
    #         etax[i] = etax[i] * -1
    ...
    # fator de escala para as inclinacoes ficarem entre +-4
    # etax1 = etax1 * (4./180)
    # etay1 = etay1 * (4./180)

    #tira os valores negativos
    
    # #correcao das series (limites e fators de escala)
    # eta[np.where(eta>0.55)] = 0.55
    # eta[np.where(eta<-0.55)] = -0.55
    # etax[np.where(etax>0.4)] = 0.4
    # etax[np.where(etax<-0.4)] = -0.4
    # etay[np.where(etay>0.4)] = 0.4
    # etay[np.where(etay<-0.4)] = -0.4
    
    # #fatores de escala das inclinacoes (ficar entre +- 4) 
    # etax = etax * -10
    # etay = etay * -10
    ...
    #     if (flage[-2,1:] == [nte*'1',nte*'1',nte*'1']).all():
        ...
        matondab.append(np.concatenate([([int(listac[-1])]),[hs,h10,hmax,tmed,thmax,hm0,tp,dp,sigma1p,sigma2p,hm01,tp1,dp1,hm02,tp2,dp2]]))
