...
def consulta_banco(boia):
    ...
    epoch_25=time.strftime('%y-%m-%d',time.gmtime((datetime.datetime(anoq,mesq-1,25,horaq,0) - datetime.datetime(1970,1,1)).total_seconds()))
...
for s in range(len(boias)):
    ...
    for i in range(len(bat)):
        ...
        if int(float(bat[i][5]))==1:
            ...
            year.append(int(float(year0[i])))
            month.append(int(float(month0[i])))
            day.append(int(float(day0[i])))
