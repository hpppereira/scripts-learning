...
elseif a==2 %Se for em águas intermediárias
    ...
    L=160;
    if h/L<(1/2) & h/L>(1/20)
        disp('A onda está se propagando em águas intermediárias, com prof(h)=10m e Compr.onda(L)=160m; quais são as equações para águas intemediárias mineiro po?!');
    end
...
for j=1:length(zf)
    ...
    for i=1:length(x)
        ...
        desvf1(i,j)=Bf(j,1)*cos(k*x(i,1)-fa*t);%*8.5; %Deslocamento vertical para águas profundas
    end   
end
...
desh10f=deshf1(:,2)+117;
desv10f=desvf1(:,2)-23;
plot(desh10f,desv10f,'r.-')
desh20f=deshf1(:,3)+117;
desv20f=desvf1(:,3)-35;
plot(desh20f,desv20f,'g.-')
desh30f=deshf1(:,4)+117;
desv30f=desvf1(:,4)-43;
plot(desh30f,desv30f,'y.-')
desh40f=deshf1(:,5)+117;
desv40f=desvf1(:,5)-50;
plot(desh40f,desv40f,'m.-')
desh50f=deshf1(:,6)+117;
desv50f=desvf1(:,6)-55;
