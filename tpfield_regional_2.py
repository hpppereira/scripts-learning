...
for k in range(0, narq):
	...
	if cfig>0:
		...
		field = map.contourf(xx,yy,m[k,:,:],levels,cmap=palette,norm=colors.normalize(vmin=0,vmax=20,clip=False))
...
#		plt.colorbar(cax=cax,orientation='horizontal') # draw colorbar
		...
		title(modelo+' Periodo de Pico - Tp (s) '+data[0:8]+' '+data[9:11]+'Z')
		savefig('tp'+data[0:8]+data[9:11]+'.png', dpi=None, facecolor='w', edgecolor='w',
...
ncfile = netcdf_file('tp'+datain[0:8]+datain[9:11]+'.nc','w')
...
lons[:] = lon
# create  variable
tp = ncfile.createVariable('hs',dtype('float32').char,('time','latitude','longitude'))
