'''
programa principal para leitura dos dados de corrente do
ADCP nas planilhas em excel do porto de tubarao
...
** nao esta funcionando - faltas alguns ajustes
...
for i in range(1,14):
	...
	sheet_3 = workbook.sheet_by_index(2) #adcp-1
	sheet_5 = workbook.sheet_by_index(6) #adcp-2
	sheet_7 = workbook.sheet_by_index(10) #adcp-3
	sheet_9 = workbook.sheet_by_index(14) #adcp-4
	...
	onda4 = np.array([[sheet_9.cell_value(r,c) for r in range(22,sheet_9.nrows)] for c in range(5,7)]).T
...
adcp4[np.where(adcp4=='')] = np.nan #coloca nan nos valores vazios


#coloca os dados em float
adcp4 = adcp4.astype(float)

adcp4[np.where(adcp4>5)] = np.nan
...
# np.savetxt('saida/proc/'+'anemo_vale.out',anemo1,delimiter=',',fmt=['%i']+2*['%.2f'],
#     header='data, ws, wd')

# np.savetxt('saida/proc/'+'adcp1_vale.out',dados1,delimiter=',',fmt=['%i']+8*['%.2f'],
#     header='data, hm0, h10, hmax, dirtp, sprtp, meandir, tp, tm02')

# np.savetxt('saida/proc/'+'adcp2_vale.out',dados2,delimiter=',',fmt=['%i']+8*['%.2f'],
#     header='data, hm0, h10, hmax, dirtp, sprtp, meandir, tp, tm02')

# np.savetxt('saida/proc/'+'adcp3_vale.out',dados3,delimiter=',',fmt=['%i']+8*['%.2f'],
#     header='data, hm0, h10, hmax, dirtp, sprtp, meandir, tp, tm02')

# np.savetxt('saida/proc/'+'adcp4_vale.out',dados4,delimiter=',',fmt=['%i']+8*['%.2f'],
#     header='data, hm0, h10, hmax, dirtp, sprtp, meandir, tp, tm02')
