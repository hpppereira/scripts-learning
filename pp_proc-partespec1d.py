...
# pathname = '/home/lioc/Documents/pnboia/dados/axys/recife/hne/'

#caminho onde estao os arquivos .HNE
# pathname = os.environ['HOME'] + '/Dropbox/lioc/dados/pnboia/axys/ondas/preproc/rio_grande/hne_rio_grande/'
...
for i in range(len(listap)):
	...
	############# aprendendo particionamento NDBC - ww3br ##############
	#** ver site da ndbc
	...
	for i in range(len(sn)):
		...
		m0.append(sn[i,1] * df)
		m2.append((sn[i,0] ** 2) * sn[i,1] * df)

		esb.append( (8 * np.pi * m2[-1]) / (9.81 * np.sqrt(m0[-1]) ) ) 

	m0 = np.array(m0)
	m2 = np.array(m2)
			

	#1) - calcular a esbeltez para cada freq

	#1.1) calcular o hs para cada freq

	#1.2) calcular o m0 e m2 para cada freq

	#2) - calcular o fs
        fs = sn[np.where(esb==max(esb))[0],0] * 0.75
        aux= [0,max(sn[:,1])]
        fsvet =[fs,fs]
        

	#3) - calcular fsu - corrigido (1.25)
        u10=30 #em m/s
        fsu= (0.9*1.25)/u10
        fsuvet=[fsu,fsu]

	pl.figure()
        pl.plot(sn[:,0],sn[:,1], fsvet, aux, fsuvet, aux)
