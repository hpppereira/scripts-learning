...
% Contributions: Izabel C. M. Nogueira
...
%
%==========================================================================
% Processamento dos dados da boia do SIODOC
%
% Dados brutos: heave, pitch, roll e compass
%
% Observacoes:
% a) Verificar com Parente ou Candella como corrige as series de pitch 
% e roll com o compass (manual ndbc96 ?? )
% b) As matrizes tem dimensao de 3020x1030 - as series estao em linha??
%
%               1  2  3  5   6   7       8,9,10, ...
% c) Formato: YYYY,MM,DD,hh,mm?,ss?, --> serie ....
% d) 32 degree of freedom
%
%==========================================================================
% Dados processados
%
% windDirectionGill - direção do vento -------------------- [67]
% windGustGill ------ intensidade da rajada --------------- [68]
% windSpeedGill ----- intensidadedo vento  ---------------- [69]
%
%==========================================================================

clear, clc, close all

% pathname = '/home/lioc/Dropbox/lioc/dados/pnboia/wavescan-siodoc/';
pathname = '/home/hp/Dropbox/database/historical/buoys/siodoc/brutos/';
pathname_proc = '/home/hp/Dropbox/database/historical/buoys/siodoc/proc/';


load ([pathname,'heave.mat']);
load ([pathname,'pitch.mat']);
load ([pathname,'roll.mat']);
load ([pathname,'compass.mat']);

ddproc = load([pathname_proc,'janis_data.dat']);

%acha indices de 1 mes
ano1 = 2014;
mes1 = 11;

%acha os indices do mes e ano
indmes = find(heave(:,2)==mes1);
indmesproc = find(ddproc(:,2)==mes1 & ddproc(:,3)==ano1);

%matriz com series temporais
heave_mat = heave(indmes,7:end);
pitch_mat = pitch(indmes,7:end);
roll_mat = roll(indmes,7:end);
compass_mat = compass(indmes,7:end);
wd = ddproc(indmesproc,67); %direcao do vento
ws = ddproc(indmesproc,69); %intensidade do vento

dmag = - 23;

%corrige dec mag do vento e bussola
wd = wd + dmag;
compass_mat = compass_mat + dmag;

%valores do compass em radianos
compass_rad = compass_mat .* (pi/180);

%??
compass_rad1 = atan(sin(compass_rad) ./ cos(compass_rad));

%correcao do compass (NDBC 96, pg.14)
%pitch
pitch_EW = ( (sin(compass_rad1) .* sin(pitch_mat)) ./ cos(pitch_mat) ) -  ( (cos(compass_rad1) .* sin(roll_mat)) ./ (cos(pitch_mat) .* cos(roll_mat)) );
%roll
roll_NS = ( (cos(compass_rad1) .* sin(pitch_mat)) ./ cos(pitch_mat) ) +  ( (sin(compass_rad1) .* sin(roll_mat)) ./ (cos(pitch_mat) .* cos(roll_mat)) );

%inverter aqui caso precise
roll_mat = pitch_EW;
pitch_mat = roll_NS;
...
mes = mes1;
ano = heave(indmes(1),1) - 2000; %para ficar, p.ex 14, em vez de 2014
...
%lista arquivos
% arq=load([pathname1 , list_month]);
% arqp=num2str(arq); %vetor com nome dos arquivos em string

%numero de arquivos a serem processados (length(arqp) ou '1'
ik = length(indmes);
...
dire = zeros(10,ik); %direcao (2 valores, ate  5 faixas)
espe = zeros(10,ik); %espectros (2 valores, ate 5 faixas)
energ = zeros(10,ik); %Hm0 + 4 energias (uma por faixa), valor zero, 4 tp (um por faixa)
...
%   carrega arquivos para processamento em batelada
...
%     [n1, n2, n3]=loadhne(arq1);
    ...
    %consistencia de comprimento do vetor
    if length(eta) < 1024
        ...
        qq1 = spectrum(eta,nfft,nfft/2,'welch');

        %correcao do fator de escala
        qq1 = 2 * dt * qq1(2:end,1); 
        ...
        faixa1 = [2:5]'; faixa11 = faixa1; %faixa 1 -- 32s - 12.8s
        faixa2 = [6:9]'; faixa22 = faixa2; %faixa 2 -- 10.6 - 7.1s
        faixa3 = [10:16]'; faixa33 = faixa3; %faixa 3 -- 6.4s - 4s
        faixa4 = [17:length(qq1)]'; faixa44 = faixa4; %faixa 4 -- 3.7s - 1.6s
...
%matriz de saida a cada 3 horas (para a pleds)
dire1 = dire(:,1:3:end); 
espe1 = espe(:,1:3:end); 
...
%pos processamento para resolver o problema do ruido em baixa frequencia
%utilizado para as series de aceleracao do GX3
