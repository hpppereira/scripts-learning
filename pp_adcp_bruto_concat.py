		...
		#verifica se tem o arquivo .whd com as datas
			...
			for arquivo in range(len(lista)): #range(len(lista))
				...
				if len(eta) < 1024:
					...
					#data em numero inteiro - para salva .out
					...
					a = zip(eta,etax,etay)
					np.savetxt('out/bruto/series/adcp'+str(adcp)+'_'+str(datai)+'.out',a,fmt='%.2f',delimiter='\t')
...
#salva arquivos
# pl.savetxt(os.environ['HOME'] + '/Dropbox/ww3vale/Geral/TU/rot/out/bruto/parametros/lioc_adcp1.out',paramw1,\
# 	delimiter=',',fmt=['%i']+5*['%.2f'],header='data,hs,hmax,hm0,tp,dp')
# pl.savetxt(os.environ['HOME'] + '/Dropbox/ww3vale/Geral/TU/rot/out/bruto/parametros/lioc_adcp2.out',paramw2,\
# 	delimiter=',',fmt=['%i']+5*['%.2f'],header='data,hs,hmax,hm0,tp,dp')
# pl.savetxt(os.environ['HOME'] + '/Dropbox/ww3vale/Geral/TU/rot/out/bruto/parametros/lioc_adcp3.out',paramw3,\
# 	delimiter=',',fmt=['%i']+5*['%.2f'],header='data,hs,hmax,hm0,tp,dp')
# pl.savetxt(os.environ['HOME'] + '/Dropbox/ww3vale/Geral/TU/rot/out/bruto/parametros/lioc_adcp4.out',paramw4,\
# 	delimiter=',',fmt=['%i']+5*['%.2f'],header='data,hs,hmax,hm0,tp,dp')
