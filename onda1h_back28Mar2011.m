...
%     ou não direcionais conforme definido no Parametreo pdir
% 
% ######################################################################
%
...
%  etaEW eh uma serie de 1024 (ou mais) pontos de inclinacao leste
%  etaNS eh uma seerie de 1024 (ou mais) pontos de inclinacao norte
%
...
%% Alterações
%>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
% 18/Mar/2011 - ( Ricardo Campos / Eric O. Ribeiro).
%>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
%%
...
%========================================================================
[arqsai,pafsai]=uiputfile([pafdad,'*.out'],'Entre com nome do arquivo de saida com os parametros');
...
prompt={'Os dados sao direcionais (entre com o valor 1) ou nao-direcionais (entre com o valor 0):',...
        'Boia Wavescan-GOM (digite 5), Boia Triaxys (digite 4), Boia do Merenda no ES (digite 3), Waverider (digite 2),da P-18 (digite 1) ou a do SEGEN/CONSUB (digite 0), deixe em branco caso seja nao-direcional:',...
        ...
        'Valor da Altura Significativa mínima para tentar identificar mais de um pico',...
        'Tratamento direcional 1D (digite 1). Tratamento direcional 1D e 2D com Método de Máxima Entropia (digite 2)'};
%def={'1',' ',' ',     ' ',       ' ',       ' ',   ' ',' ',  ' ', ' '}; %Padroes para qualquer dado de boia.
%def={'1','4',' ','0.78','-24.39','-44.40','2008',' ','32','-23','0.85'}; %Padroes para a boia Triaxys.
def={'1','5',' ','0.5','26.7','-90.45','2009',' ','32','0.5','1e3','0.85','2'}; %Padroes para a boia Wavescan.
...
% Tipo de boia utilizado na medicao.
...
% Chaveamento para leitura específica de PAT-3 e Pearl Marine
...
prof=str2num(char(answer(11)));

% Hs minimo. Evita problemas na trat_dir_emem e sele_pico
hsminimo=str2num(char(answer(12)));

% Flag para decidir entre tratamento direcional 1D (trat_dir_fft...), mais
% rápido, ou tratamento direcional 1D E 2D (trat_dir_EMEM) mais demorado.
decidespec=str2num(char(answer(13)));
...
% VEPK1 Parametreo Espalhamento do Pico 1 (Mais Energetico)
...
% VEPK2  Parametreo Espalhamento do Pico 2. O segundo mais Energetico. 
...
% VEPK3  Parametreo Espalhamento do Pico 3. O menos Energetico.
...
while ~isempty(arqdwv) % Teste para identificar se ainda existe algum arquivo
  ...
  if pdir == 1
    ...
    if dboia == 1
      ...
      [etaEW etaNS]=rota_boia_p18(roll,pitch,compas);
    ...
    if dboia == 4
      % Leitura da boia Axys. Arquivos sao referentes a elevacao e deslocamentos
      [heave,etaEW,etaNS,ano,mes,dia,hora,min,sdata,stime]=le_boiaBS(arqdwv); % mudar o nome para le_boiaAxys(arqdwv)
    end
    ...
    if t==0; % Chaveamento para leitura específica de PAT-3 e Pearl Marine
  ...
  %========================================================================
   if pdir
    ...
    % Calculando o espectro direcional 2D pela rotina WAFO (método de
    % maxima entropia EMEM)
    if decidespec==2
      if VAVH >= hsminimo  % Hs minimo para aplicar o EMEM. Ricardo 11/02
        [VF2D, VDIR2D, VESP2D] = trat_dir_EMEM(DT,heave,etaEW,etaNS,DONV);
      else
        VF2D=-999;VDIR2D=-999;VESP2D=-999;
      end
    end
    % Calculando a direção pelo Metodo de Longuet-Higgins (pelo artigo do Tucker (1989), ocean enginnering vol. 16, pp.173-192.)
    %[VD,VPED,VNUM,VESP,VEPK] = trat_dir(VF,noverlap,w,heave,etaEW,etaNS,DT,DONV,kVTPK,dboia);
    %[VD,VPED,VNUM,VESP,VEPK] = trat_dir_fft(VF,noverlap,w,heave,etaEW,etaNS,DT,DONV,kVTPK,nfft,dboia);
    [VD,VPED,VNUM,VESP,VEPK,A1,B1,A2,B2,Rcheck] = trat_dir_fft_ndbc9601(VF,noverlap,w,heave,etaEW,etaNS,DT,DONV,kVTPK,nfft,dboia,mgl,prof);
    %[VD,VPED,VNUM,VESP,VEPK] = trat_dir_fft_tucker1989(VF,noverlap,w,heave,etaEW,etaNS,DT,DONV,kVTPK,nfft,dboia); % Não apresentou bons resultados!!! Função igual ao artigo do Tucker 1989.
  ...
  %========================================================================
  plot_treshold  = 0.3; % Limite inferior de Hs para plotar o Espectro
  if pdir
  ...
  % escrita do espectro 1D, resultado da trat_dir_fft
  ...
  if VAVH>hsminimo
    ...
    fprintf(fid5,'%s\n','% Foi utilizada a rotina dat2dspec do pacote WAFO com metodo EMEM (NFFT=128 pontos)');
    ...
    if 4*sqrt(VMTA) > 0
      ...
      if decidespec==2
        if VAVH>hsminimo
          ...
          fig=fig+1;
          figure(fig)
          VDIR2D(length(VDIR2D)+1,1)=VDIR2D(1);
          VESP2D(:,length(VESP2D)+1)=VESP2D(:,1);
