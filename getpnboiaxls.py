...
#pathname for the existed file
pathname = os.environ['HOME'] + '/Dropbox/pnboia/data/realtime/xls/'
...
nameout = [
		   ...
		   # "PNBOIA_VIX_xls",
		   # "PNBOIA_PSG_xls",
		   # "PNBOIA_RCF_xls", #em manutencao
		   # "PNBOIA_BGA_xls",
		   # "PNBOIA_SAN_xls"
		   ]


dmag = [-17,-23,-23,-22,-23,-22]

#verify the actual month
...
for boia in boias:
	...
	try:
		...
		#pathname out file (same name)
		arq = pathname + 'xls/' + nameout[cont] + '.xls'

		#carrega arquivo antigo concatenado 
		old = pd.read_table(pathname + nameout[cont] + '.csv',sep=',',parse_dates=['date'],index_col=['date'])

		#baixa dado
		site_adress = "https://www.mar.mil.br/dhn/chm/meteo/prev/dados/pnboia/"+mes+"_ARGOS_"+boia+".xls"
		# site = urllib.request.urlretrieve(site_adress,arq)
		...
		#converte os nan em string par nan de verdade
		df0[df0 == 'nan'] = np.nan
		df1[df1 == 'nan'] = np.nan
		...
		df.to_csv(pathname + nameout[cont] + '.csv',na_rep='nan')
