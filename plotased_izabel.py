...
cenario = 'mediana'
filename_mod = 'mod_latlon_grid_Hsprc10.csv'
...
for i in range(len(sed)):
	if sed.graocm[i] <= 0.05: #cm
		...
		#print aux
		ucw.append( (-abs(aux) ** (2.0/3)) * (sed.graocm[i]**(1.0/3)) * (mod.tp[i]**(1.0/3)))
	else:
		aux = 1.090 * g * (s - 1)
		#print aux
		ucw.append( (-abs(aux) ** (4.0/7)) * (sed.graocm[i]**(3.0/7)) * (mod.tp[i]**(1.0/7)))
...
mod.to_csv('out/' + cenario + '/' + 'errado_ucw_' + filename_mod[:-4] + '.csv')

sed.to_csv('out/' + cenario + '/' + 'errado_sed_' + filename_mod[:-4] + '.csv')
