...
# -----------------------------------------------------------------
# Resolution (minutes): The same of CFSR
...
dife=abs(lonr[:]-loni)
aloni=dife.argmin()
if loni-lonr[aloni] < 0:
	aloni=aloni-1

dife=abs(lonr[:]-lonf)
alonf=dife.argmin()
if lonf-lonr[alonf] > 0:
	alonf=alonf+1

lonr=lonr[aloni:alonf+1]
...
lat = fu.variables['ETOPO01_Y'][:]
indlati=find(abs(lat-latr[0])==min(abs(lat-latr[0])))
indlatf=find(abs(lat-latr[-1])==min(abs(lat-latr[-1])))
indlatf=indlatf+1
lat = fu.variables['ETOPO01_Y'][indlati:indlatf+1]


lon = fu.variables['ETOPO01_X'][:]
indloni=find(abs(lon-lonr[0])==min(abs(lon-lonr[0])))
indlonf=find(abs(lon-lonr[-1])==min(abs(lon-lonr[-1])))
indlonf=indlonf+1
lon = fu.variables['ETOPO01_X'][indloni:indlonf+1]

b = fu.variables['ROSE'][indlati:indlatf+1,indloni:indlonf+1]
fu.close()
# -------------------------------------------
