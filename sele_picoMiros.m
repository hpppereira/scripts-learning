function[Spico,fSpico,dSpico,Scava,fScava,dScava] = sele_picoMiros(VS,VD,VF,VSPK,ngl,pdir,deltaf,arqdwv,VMTA,hsmin)
%
% Funcao para selecao de picos espectrais  
% Entrada:
%    VS = Espectro de energia, m2/Hz,
%   VD = Espectro de direções, graus,
%   VF = vetor de frequencia, Hz,
%   VSPK = Valor de densidade espectral do pico de energia,
%   ngl =  número de graus de liberdade,
%   pdir = indicador de direcionalidade 0 = não e  1 = sim,
%   deltaf = intervalo de tempo de amostragens, s,
%   arqdwv = noem do arquivo de leitura,
%   VMTA = momento de ordem zero do espectro, m2,
%   hsminimo = Hs mínimo a ser considerado no selecão de pico,
%
% Saída:
% Spico = valores dos picos do espectro,
% fSpico = frequencias dos picos do espectro,
% dSpico = direções dos picos do espectro,
% Scava = valores das cavas do espectro,
% fScava = frequencias das cavas do espectro,
% dScava = direções das cavas do espectro,
%
...
% 10/mai/2011, EOR - Essa função foi originada pela sele_picoP40.

% 01/Jun/2011, EOR - Inclusão nos argumentos de ntrada das variaveis VMTA e hsminimo.

if 4*sqrt(VMTA) < hsmin;
  [Spico,posicao]=max(VS);
  fSpico=VF(posicao);
  dSpico=VD(posicao);
