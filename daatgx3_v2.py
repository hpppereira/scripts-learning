...
pathname = os.environ['HOME'] + '/Dropbox/DAAT_PLEDS/DAAT_PLEDS_python/ilha_rasa/dados/'
...
#     delimiter=',',skiprows=4,dtype=str)
def parse(yr, mon, day, hr):
    yr, mon, day, hr = [int(x) for x in [yr, mon, day, hr]]
    dt = datetime(year=yr, month=mon, day=day, hour=hr)
    return dt
kw = dict(index_col = 0, header = None, delim_whitespace = True, parse_dates = {'datetime':[0,1,2,3]}, date_parser = parse)

aew = pd.read_table(pathname + 'EW.dat',
	**kw)

ans = pd.read_table(pathname + 'NS.dat',
	**kw)

av = pd.read_table(pathname + 'V.dat',
	**kw)
...
#vetor de data
# data = pd.to_datetime([aew[i,0].strip('"') for i in range(len(aew))])
...
# av = av[:,2:].astype(np.float).T # * -9.81 #m/s2

#declinacao magnetica
dmag = -23

#inicia calculo do tempo de processamento
tic = timem.clock()

h = 200 #profundidade
nfft = 64 #numero de dados para a fft (p/ nlin=1312: 32gl;nfft=82, 16gl;nfft=164, 8gl;nfft=328)
fs = 1.0 #freq de amostragem
dt = 1 / fs #int amostragem
...
for ik in range(0,720): #len(listap)):

    print (str(ik) + ' - DAAT - ' + str(listap[ik]))
...
win=7
meth='mean'
dire1[0,:] = pd.rolling_mean(dire1[0,:], win, how=meth)
dire1[2,:] = pd.rolling_mean(dire1[2,:], win, how=meth)
dire1[4,:] = pd.rolling_mean(dire1[4,:], win, how=meth)
dire1[6,:] = pd.rolling_mean(dire1[6,:], win, how=meth)

pathname1 = os.environ['HOME'] + '/Dropbox/DAAT_PLEDS/DAAT_PLEDS_python/ilha_rasa/dados/' # '/Dropbox/siodoc/dados/brutos/'
# periodo = '2014-10'
# dmag = -23


#carrega arquivos .mat
wind = scipy.io.loadmat(pathname1 + 'wind.mat')
data_all1 = wind['wind'][:,[0,1,2,3,4,5]]
