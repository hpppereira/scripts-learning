...
hs=zeros((nt,npo),'f');freqp=zeros((nt,npo),'f');tp=zeros((nt,npo),'f');dp=zeros((nt,npo),'f');psprd=zeros((nt,npo),'f')
...
for j in range(0,npo):
	...
	title(' Wavewatch III. Point'+repr(j+1)+': Lat'+repr(lat[j])[0:8]+' Lon'+repr(lon[j])[0:8])
...
for i in range(0,npo):
	...
	vf.write('% WAVEWATCH III Point'+repr(i+1)+': Lat'+repr(lat[i])[0:8]+' Lon'+repr(lon[i])[0:8])
	vf.write('\n')	
