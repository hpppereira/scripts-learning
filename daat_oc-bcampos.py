# DAAT adaptada para o congresso oceans para os dados
# da bacia de campos , a cada 3 horas
...
def daat1(pathname,listap,nfft,fs,ncol):

	tic = time.clock()

	#define as faixas de frequencia para a DAAT (32 gl)
	f1d = range(3,6)
	f2d = range(6,10)
	f3d = range(10,18)
	f4d = range(18,32)
	...
	for ik in range(len(listap)):
	    ...
	    t, eta, etax, etay = np.loadtxt(pathname + lista[ik], unpack=True)
	    ...
	    #espectros nas 4 faixas - variavel com o gl
	    # ww55[1] = sum(qq1[1:5])
	    # ww55[2] = sum(qq1[5:7])
	    # ww55[3] = sum(qq1[7:14])
	    ...
	    # picos1 = np.array([13*3,9*3,6*3,4*3])
