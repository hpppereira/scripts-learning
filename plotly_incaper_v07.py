...
for v in vv:
    ...
    if v == 'windspd':
      ...
      fig1 = ff.create_quiver(np.arange(len(df.index)), 
...
#      quiver.layout.yaxis1.update({'anchor': 'x1', 'domain': [.55, 1]})
      ...
      aa = go.Figure(data=fig1['data'], layout=layout)
      #plot(fig1, filename='index1.html', show_link=False)

      fig.append_trace(aa, ll, 1)
