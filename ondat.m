...
function [Hs,H10,Hmax,THmax,Tmed]=onda_tempo(n1,fs,h)

%intervalo de amostragem
dt = 1 / fs;

%cria vetor de tempo
t=(linspace(dt,length(n1)*dt,length(n1)))';
...
for i=1:length(n1)-1     
    ...
    if n1(i)<nm && n1(i+1)>nm %Condicao de zero ascendente       
        
        contza=contza+1; %conta zero ascendente
        
        x=[t(i) t(i+1)]; %Cria vetor x(1x2) para fazer o calculo da regressao linear 
        
        y=[n1(i) n1(i+1)]; %Cria vetor y(1x2) para fazer o calculo da regressao linear
...
for i=1:length(ind_nm)-1
    ...
    elmin(i,1)=min(n1(ind_nm(i,1):ind_nm(i+1,1))); %Acha elevacao minima entre 2 zeros ascendentes
    
    elmax(i,1)=max(n1(ind_nm(i,1):ind_nm(i+1,1))); %Acha elevacao maxima entre 2 zeros ascendentes
