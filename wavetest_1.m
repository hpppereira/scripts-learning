...
figure
%--- Plot time series
subplot('position',[0.1 0.55 0.65 0.32])
plot(time,sst1,'b'), grid on
set(gca,'XLim',xlim(:))
xlabel('Time (seconds)')
ylabel('Water level (m)')
eval(['title(num2str(' arquivo(1:8) '' '))'])
hold off

%--- Contour plot wavelet power spectrum
subplot('position',[0.1 0.07 0.65 0.38])
levels = [0:1:max(max(power))]; %[0.0625,0.125,0.25,0.5,1,2,4,8,16] ;
...
subplot('position',[0.77 0.07 0.2 0.38])
plot(global_ws,log2(period))
hold on
plot(global_signif,log2(period),'--')
hold off
xlabel('Power (m^2)')
title('c) Wavelet Spectrum')
set(gca,'YLim',log2([min(period),max(period)]), ...
	'YDir','reverse', ...
	'YTick',log2(Yticks(:)), ...
	'YTickLabel','')
