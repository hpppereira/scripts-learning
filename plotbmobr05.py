...
pathname = os.environ['HOME'] + '/Dropbox/projects/BMOP/Processamento/dados/CF1_BMOBR05_2016Fev/op/'
pathfig = os.environ['HOME'] + '/Dropbox/projects/BMOP/Processamento/fig/BMOBR05_CF1/'

#carrega os dados da boia
dd = pd.read_csv(pathname + 'Dados_BMOBR05.csv', index_col='date', parse_dates=True)

#carrega os dados do geoforce
ga = pd.read_csv(pathname + 'Geoforce_BMOBR05_A.csv',sep=',',parse_dates=['date'],index_col=['date'])
gb = pd.read_csv(pathname + 'Geoforce_BMOBR05_B.csv',sep=',',parse_dates=['date'],index_col=['date'])

#corrige utc para local nos dados do geoforce
ga.index = ga.index - datetime.timedelta(hours=3)
gb.index = gb.index - datetime.timedelta(hours=3)

#converte posicoes para UTM
aux_dd = np.array([utm.from_latlon(dd.lat[i],dd.lon[i]) for i in range(len(dd))])
aux_ga = np.array([utm.from_latlon(ga.lat[i],ga.lon[i]) for i in range(len(ga))])
aux_gb = np.array([utm.from_latlon(gb.lat[i],gb.lon[i]) for i in range(len(gb))])

dd['lonu'],dd['latu'] = aux_dd[:,[0,1]].T
ga['lonu'],ga['latu'] = aux_ga[:,[0,1]].T
gb['lonu'],gb['latu'] = aux_gb[:,[0,1]].T
...
ax2.plot(ga.lonu[-3:],ga.latu[-3:],'k*') #geoforce A
ax2.plot(gb.lonu[-3:],gb.latu[-3:],'k*') #geoforce B
ax2.text(ga.lonu[-3],ga.latu[-3],str(ga.index[-3])[0:-6]+'h')
ax2.text(ga.lonu[-2],ga.latu[-2],str(ga.index[-2])[0:-6]+'h')
ax2.text(ga.lonu[-1],ga.latu[-1],str(ga.index[-1])[0:-6]+'h')
ax = pl.gca()
