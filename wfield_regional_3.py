# The program reads the output wave field files from wavewatch III v 3.14. Regional grid 3
...
for k in range(0, narq):
	...
	if cfig>0:
		...
		map.contourf(xx,yy,m[k,:,:],levels,cmap=palette,norm=colors.normalize(vmin=0,vmax=4,clip=False))

		ax = plt.gca()
		pos = ax.get_position()
		l, b, w, h = pos.bounds
		cax = plt.axes([l+w+0.02, b+0.01, 0.03, h-0.01]) # setup colorbar axes.
		plt.colorbar(cax=cax) # draw colorbar
		plt.axes(ax)  # make the original axes current again

		title(modelo+' Significant Wave Height (m) '+data[0:8]+' '+data[9:11]+'Z')
