...
Izabel C M Nogueira
Laboratorio de Instrumentacao Oceanografica

Ultima modificacao: 15/08/2015
...
for boia in boias:
	...
	arq = arqs[-1]
	#arq = boia + '_2015083110.xls' #processa dado xls especifico (p.ex: mensal)
	...
	#pega apenas os dados das ultimas 30 horas
	dd0 = dd0[-30:]
	dd1 = dd1[-30:]
	...
	ddvento = np.loadtxt(pathname + 'LIOc/' + boia + '_vento.out',delimiter=',',skiprows=1)
	ddonda = np.loadtxt(pathname + 'LIOc/' + boia + '_onda.out',delimiter=',',skiprows=1)
	...
	#salva o arquivo do mes concatenado (mudar de 'concat' para 'mat' quando for para salvar algum arquivo)
	...
	#tj = len(dt1)
	#tj = 500
	#tj = pl.find(dt1i>201508310000)[0]

	#cria texto para mandar por email atraves do infowaves
	text = (local[ff-1]	+                                      '\n' +
			"Boia: AXYS - " + boia +                           '\n' +		
			dt.datetime.strftime(dt1[-1],'%d/%m/%Y - %Hh') +   '\n' +
			"WS1 = "  + '%.1f' %ws1[-1]  + " m/s" +            '\n' +
			"WD1 = "  + '%.i'  %wd1[-1]  + " deg" +            '\n' +
			"WS2 = "  + '%.1f' %ws2[-1]  + " m/s" +            '\n' +
			"WD2 = "  + '%.i'  %wd2[-1]  + " deg" +            '\n' +
	...
	outfile = codecs.open(os.environ['HOME'] + '/Dropbox/pnboia/rot/out/email/' + boia + '.mail','w','utf-8')
...
pl.savefig(pathnameout + 'report_' + data + '.png')

#copia o report para a pasta do ww3vale
#os.system('cp ' + os.environ['HOME'] + '/Dropbox/pnboia/reports/report_' + data + '.png ' + os.environ['HOME'] + '/Dropbox/ww3vale/reports')
#os.system('cp ' + os.environ['HOME'] + '/Dropbox/pnboia/reports/report_' + data + '.png ' + os.environ['HOME'] + '/Dropbox/nemes/reports')
