function [VMTA1,VMTB1,VMTC1,VMTD1,VMTE1,VCAR1,VTPK1,VSPK1,VSPR1,s1_VSPR1,s1_VTPK1,s2_VSPR1,s2_VTPK1,VPED1,VMTA2,VMTB2,VMTC2,VMTD2,VMTE2,VCAR2,VTPK2,VSPK2,VSPR2,s1_VSPR2,s1_VTPK2,s2_VSPR2,s2_VTPK2,VPED2,VMTA3,VMTB3,VMTC3,VMTD3,VMTE3,VCAR3,VTPK3,VSPK3,VSPR3,s1_VSPR3,s1_VTPK3,s2_VSPR3,s2_VTPK3,VPED3] = momentos_rev2(fScava,Spico,fSpico,dSpico,VF,VS,NDT,VMTA,VMTB,VMTC,VMTD,VMTE,VTPK,VSPK,VSPR,s1_VSPR,s2_VSPR,VPED,VESP,s1,s2,pdir);
%
...
%             Revisão para incluir o calculo do spreading VSPR e fatores s1
...
for k=1:length(fScava)
   ...
   if length(fScava) == 1
      ...
      if pdir
         VPED1=VPED;VSPR1=VSPR;s1_VSPR1=s1_VSPR;s2_VSPR1=s2_VSPR;s1_VTPK1=s1(find(VF == fSpico(1)));s2_VTPK1=s2(find(VF == fSpico(1)));
      else
         VPED1=-999.99;VSPR1=-999.99;s1_VSPR1=-999.99;s1_VTPK1=-999.99;s2_VSPR1=-999.99;s2_VTPK1=-999.99;
      end
      VMTA2=-999.99;VMTB2=-999.99;VMTC2=-999.99;VMTD2=-999.99;VMTE2=-999.99;VCAR2=-999.99;
      VTPK2=-999.99;VSPK2=-999.99;VSPR2=-999.99;VPED2=-999.99;s1_VSPR2=-999.99;s1_VTPK2=-999.99;s2_VSPR2=-999.99;s2_VTPK2=-999.99;
      VMTA3=-999.99;VMTB3=-999.99;VMTC3=-999.99;VMTD3=-999.99;VMTE3=-999.99;VCAR3=-999.99;
      VTPK3=-999.99;VSPK3=-999.99;VSPR3=-999.99;VPED3=-999.99;s1_VSPR3=-999.99;s1_VTPK3=-999.99;s2_VSPR3=-999.99;s2_VTPK3=-999.99;
   ...
   elseif length(fScava) == 2
      ...
      if pdir
         ...
         % VSPR1=VESP(find(VS == auxS(2))); VSPR2=VESP(find(VS == auxS(1)));
         ...
         VSPR1=sum(VESP(nfk(nk(2)):nfk(nk(2)+1)).*E1); % Conforme seção "Circular rms spreading" do paper Forristal&Ewans(1998) equacao (80)
                                                       % e tambem no livro Tucker&Pitt(2001) equação (7.2-21)
         s1_VSPR1=sum(s1(nfk(nk(2)):nfk(nk(2)+1)).*E1);
         s1_VTPK1=s1(find(VF == fSpico(1)));
         s2_VSPR1=sum(s2(nfk(nk(2)):nfk(nk(2)+1)).*E1);
         ...
         VSPR2=sum(VESP(nfk(nk(1)):nfk(nk(1)+1)).*E2);% Conforme seção "Circular rms spreading" do paper Forristal&Ewans(1998) equacao (80)
                                                      % e tambem no livro Tucker&Pitt(2001) equação (7.2-21) 
         s1_VSPR2=sum(s1(nfk(nk(1)):nfk(nk(1)+1)).*E2);
         s1_VTPK2=s1(find(VF == fSpico(2)));
         s2_VSPR2=sum(s2(nfk(nk(1)):nfk(nk(1)+1)).*E2);
         s2_VTPK2=s2(find(VF == fSpico(2)));
      else
         VPED1=-999.99;VSPR1=-999.99;s1_VSPR1=-999.99;s1_VTPK1=-999.99;s2_VSPR1=-999.99;s2_VTPK1=-999.99;
         VPED2=-999.99;VSPR2=-999.99;s1_VSPR2=-999.99;s1_VTPK2=-999.99;s2_VSPR2=-999.99;s2_VTPK2=-999.99;
      end
      VMTA3=-999.99;VMTB3=-999.99;VMTC3=-999.99;VMTD3=-999.99;VMTE3=-999.99;VCAR3=-999.99;
      VTPK3=-999.99;VSPK3=-999.99;VSPR3=-999.99;VPED3=-999.99;s1_VSPR3=-999.99;s1_VTPK3=-999.99;s2_VSPR3=-999.99;s2_VTPK3=-999.99;
   elseif length(fScava) == 3
      ...
      if pdir
         ...
         % Espalhamento medio da cada região de pico
         % Opção 1: selecionando o desvio padrao circular VESP exatamento nos picos 
         % VSPR1=VESP(find(VS == auxS(3))); VSPR2=VESP(find(VS == auxS(3)));VSPR3=VESP(find(VS == auxS(1)));
         % Opção 2: utilizar uma media ponderada pelo espectro (proposta paper Forristall&Ewans)
         df=VF(3)-VF(2);
         E1=VS(nfk(nk(3)):nfk(nk(3)+1)).*df/VMTA1; % Calculo do espectro omni-direcional normalizado pela area espectral
         VSPR1=sum(VESP(nfk(nk(3)):nfk(nk(3)+1)).*E1); % Conforme seção "Circular rms spreading" do paper Forristal&Ewans(1998) equacao (80)
                                                       % e tambem no livro Tucker&Pitt(2001) equação (7.2-21)
         s1_VSPR1=sum(s1(nfk(nk(3)):nfk(nk(3)+1)).*E1);
         s1_VTPK1=s1(find(VF == fSpico(1)));
         s2_VSPR1=sum(s2(nfk(nk(3)):nfk(nk(3)+1)).*E1);
         s2_VTPK1=s2(find(VF == fSpico(1)));
         E2=VS(nfk(nk(2)):nfk(nk(2)+1)).*df/VMTA2; % Calculo do espectro omni-direcional normalizado pela area espectral
         VSPR2=sum(VESP(nfk(nk(2)):nfk(nk(2)+1)).*E2); % Conforme seção "Circular rms spreading" do paper Forristal&Ewans(1998) equacao (80)
                                                       % e tambem no livro Tucker&Pitt(2001) equação (7.2-21)
         s1_VSPR2=sum(s1(nfk(nk(2)):nfk(nk(2)+1)).*E2);
         s1_VTPK2=s1(find(VF == fSpico(2)));
         s2_VSPR2=sum(s2(nfk(nk(2)):nfk(nk(2)+1)).*E2);
         s2_VTPK2=s2(find(VF == fSpico(2)));
         E3=VS(nfk(nk(1)):nfk(nk(1)+1)).*df/VMTA3; % Calculo do espectro omni-direcional normalizado pela area espectral
         VSPR3=sum(VESP(nfk(nk(1)):nfk(nk(1)+1)).*E3); % Conforme seção "Circular rms spreading" do paper Forristal&Ewans(1998) equacao (80)
                                                       % e tambem no livro Tucker&Pitt(2001) equação (7.2-21)
         s1_VSPR3=sum(s1(nfk(nk(1)):nfk(nk(1)+1)).*E3);
         s1_VTPK3=s1(find(VF == fSpico(3)));
         s2_VSPR3=sum(s2(nfk(nk(1)):nfk(nk(1)+1)).*E3);
         s2_VTPK3=s2(find(VF == fSpico(3)));
      else
         VSPR1=-999.99;VPED1=-999.99;s1_VSPR1=-999.99;s2_VSPR1=-999.99;s2_VTPK1=-999.99;
         VSPR2=-999.99;VPED2=-999.99;s1_VSPR2=-999.99;s2_VSPR2=-999.99;s2_VTPK2=-999.99;
         VSPR3=-999.99;VPED3=-999.99;s1_VSPR3=-999.99;s2_VSPR3=-999.99;s2_VTPK3=-999.99;
      end
      VMTA2=mo0(nk(2));VMTB2=mo1(nk(2));VMTC2=mo2(nk(2));VMTD2=mo3(nk(2));VMTE2=mo4(nk(2));VCAR2=4*sqrt(VMTA2);
      VTPK2=1./fSpico(2);VSPK2=auxS(2);
      VMTA3=mo0(nk(1));VMTB3=mo1(nk(1));VMTC3=mo2(nk(1));VMTD3=mo3(nk(1));VMTE3=mo4(nk(1));VCAR3=4*sqrt(VMTA3);
