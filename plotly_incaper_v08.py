...
fig = tools.make_subplots(rows=len(vv), cols=1, shared_xaxes=False,
...
plot(fig, filename='index_scatter.html', show_link=False)
...
layout = go.Layout(
    title="Intensidade e Direção do Vento",
    autosize=False,
    width=1100,
    height=400,
    showlegend=False,
...
plot(fig, filename='index_stick.html', show_link=False)



##########################################################3
# contatena os 2 htmls (index_stick + index_scatter)

f1 = open('index_scatter.html')
f2 = open('index_stick.html')

p1 = f1.read()[:-23] #apenas com o header
p2 = f2.read()[199:] #apenas o final
