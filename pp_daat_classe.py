...
*verificar diferenca de tempo de processamento
...
#################################################
#dados de entrada
pathname = os.environ['HOME'] + '/Dropbox/ww3vale/Geral/TU/rot/out/bruto/series/'

#lista arquivos do diretorio atual do periodo escolhido
lista = []

for f in os.listdir(pathname):
	if f.startswith('adcp1_201304'):
		lista.append(f)
...
def daat0():
	...
	return wavecos, wavesen, out1, out2, mm
...
def daat1(co,dd,dc,dt,out1,out2,mm,m):
...
#inicia o loop do processamento em batelada

#carrega wavecos e wavesen para maxima entropia
#wavecos, wavesen = daat1()
wavecos, wavesen, out1, out2, mm = daat0()
...
for ik in range(len(lista)):
    ...
    faixa2 = np.array([4,5,6])
    faixa3 = np.array([6,7,8])
    faixa4 = np.array([8,9,10])
    ...
    for iwq in range(4):
        ...
        #chama funcao daat1 e daat2
        tet2, sp2 = daat1(co,dd,dc,dt,out1,out2,mm,m)
        espe, dire, energ = daat2(sp2,tet2)
