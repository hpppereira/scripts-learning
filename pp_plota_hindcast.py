...
pathname  = '/media/hp/lioc6/hindcastVale/VIX_60X45/'
direm = np.sort(os.listdir(pathname))

#diretorio de onde estao os resultados do ADCP02 3 4
pathname2  = '/media/hp/lioc6/hindcastVale/BES_65X45/'
...
for dto in direm:
	...
	madcp01= np.loadtxt(pathname + dto + '/table_point.out',skiprows=7,usecols=(0,1,3,2))
...
ax = new_axes()
ax.bar(dado[:,3], dado[:,1], normed=True, bins=binshs, opening=.8, edgecolor='white',nsector=8)
...
ax = new_axes()
ax.bar(dado[:,3], dado[:,2], normed=True, bins=binstp, opening=.8, edgecolor='white',nsector=8)
ax.grid(True,linewidth=1.5,linestyle='dotted')
set_legend(ax)
